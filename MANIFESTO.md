...
## The ICML 2026 Crackdown

The problem didn't go away. It got worse, and conferences started fighting back.

**March 2026**: ICML 2026 deployed a watermarking-based detection system to catch reviewers using LLMs in violation of their review policies. The results:

- **497 papers desk-rejected** — their corresponding reviewers violated the no-LLM policy
- **795 reviews (~1% of all reviews)** flagged as LLM-generated by 506 unique reviewers
- **51 reviewers** had more than half their reviews removed and were banned from the reviewer pool

ICML implemented a two-policy system: reviewers could opt into Policy A (no LLM use) or Policy B (LLM-assisted). The detection targeted Policy A reviewers who used LLMs anyway. The method embedded hidden watermark phrases in submission PDFs — if an LLM was fed the PDF and asked to generate a review, the watermark phrases would appear in the output.

The family-wise error rate was calculated at 0.0001 — extremely low false positive probability. This wasn't a dragnet. It caught people who explicitly opted into a no-LLM policy and then broke it.

Sources:
- [ICML Blog: On Violations of LLM Review Policies](https://blog.icml.cc/2026/03/18/on-violations-of-llm-review-policies/)
