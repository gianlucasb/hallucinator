...
- Local Corpus (Rust only — `hallucinator-local-corpus`; offline SQLite+FTS5, fuzzy-matched. Recent conference proceedings not yet indexed elsewhere, plus references marked safe during review. Built incrementally via `import-*` CLI subcommands — NDSS, USENIX, NSDI, OSDI (NSDI/OSDI reuse `import-usenix` — same USENIX Drupal template), IEEE S&P, CCS, ACSAC, ESORICS, RAID, ASIACCS, EuroS&P, NeurIPS, ICSE, AAAI, DEF CON, Black Hat, CVPR/ICCV/WACV (`import-cvf`, one shared CVF Open Access template), plus `import-corpus-reports` for marked-safe JSON — no online counterpart)
